  ...
  const handleAddToCart = async (item) => {
    try {
      ...
      if (response.status === 201) {
        alert(`${item.name} add to cart successfully!`);
      }

    } catch (error) {
      console.error("Error:", error);
      alert("There is an issue with the backend connection");
    }
  };
      ...
      <div className="container">
        ...
        <div className="product-grid-layout">
          {filteredProducts.map((item) => (
            <div
              key={item.id}
              className={`single-product-item ${item.stock === 0 ? 'out-of-stock' : ''}`}
            >
              ...
              <div className="product-thumb">
                <img
                  src={`/assets/images/Shop/product${item.id}.png`}
                  alt={item.name}
                />
              ...
              <div className="product-content">
                <span
                  className={`category-tag cat-${item.category.toLowerCase()}`}
                >
                  {item.category}
                </span>
